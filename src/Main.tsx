...
function Main() {
  ...
  const keyDownHandler = (e: React.KeyboardEvent) => {
    ...
    if (
      (keyPress?.Control || e.key == "Control") &&
      (keyPress?.b || e.key == "b") &&
      keyPress.isAdded == false
    ) {
      setSayHello(true);
    }
  };
    ...
    <div className="main-body" onKeyDown={keyDownHandler} onKeyUp={keyUpHandler} tabIndex={0}>
      <div className="main-container">
        ...
        <div className="command">
          ...
          <button className="command-btn">Ctrl</button>+<button className="command-btn">B</button>
        </div>
        <div className="alarm-container">
          {helloList.length > 5 && (
            <AlramBox
              idx={0}
              setData={setHelloList}
              message={"Too Many Hello!"}
              noMessage=""
              okMessage=""
              messageType="WARNING"
              title="hello-warning"
